...
export default function ReviewCarousel({ reviews }: ReviewCarouselProps) {
  ...
  return (
    <>
      {/* Reviews Grid - Staggered Masonry */}
        ...
        {visibleReviews.map((review, idx) => (
          <div
            ...
            style={{
              ...
              transform: idx % 3 === 1 ? 'translateY(-3rem)' : 'translateY(2rem)',
            }}
          >
            {/* Review Card - Superpower Style with Corner Borders */}
            <div className="bg-white rounded-xl p-7 border border-gray-200 relative overflow-hidden hover:shadow-md transition-shadow duration-300 h-full flex flex-col group">
              {/* Corner Border Accents (Top-left & Bottom-right) */}
              <div className="absolute top-0 left-0 w-6 h-6 border-t-2 border-l-2 border-[#E31C5F]/60 rounded-tl-lg group-hover:border-[#E31C5F] transition-colors duration-300" />
              <div className="absolute bottom-0 right-0 w-6 h-6 border-b-2 border-r-2 border-[#E31C5F]/60 rounded-br-lg group-hover:border-[#E31C5F] transition-colors duration-300" />
              ...
              {/* Reviewer info - Minimal and clean */}
      ...
      {/* See More Button - Always visible when there are reviews */}
      {allReviews.length > 0 && (
        <div
          className="mt-16 flex justify-center animate-fadeInUp"
          style={{ animationDelay: "900ms" }}
        >
          <button
            onClick={() => setShowAll(!showAll)}
            className="group relative px-8 py-3.5 bg-[#E31C5F] text-white rounded-lg font-semibold hover:bg-[#C4184F] transition-all duration-300 shadow-[0_4px_12px_rgba(227,28,95,0.3)] hover:shadow-[0_8px_20px_rgba(227,28,95,0.4)]"
          >
